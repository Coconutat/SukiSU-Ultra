				    ...
				    void *data)
{
	...
	for (oldn = cls->constraints; oldn; oldn = oldn->next) {
		...
		for (olde = oldn->expr; olde; olde = olde->next) {
			e = kmemdup(olde, sizeof(struct constraint_expr),
				    GFP_KERNEL);
			if (!e) {
				goto out_nomem;
			}
				      ...
				      struct policydb *old_db)
{
	...
	ret = hashtab_duplicate(&new_db->p_classes.table,
				&old_db->p_classes.table,
...
static int copy_avtab(struct avtab *new_avtab, struct avtab *old_avtab)
{
	...
	for (i = 0; i < old_avtab->nslot; i++) {
		...
		while (n) {
			p = avtab_insert_nonunique(new_avtab, &n->key,
						   &n->datum);
			if (!p) {
				ret = -ENOMEM;
				goto out_free;
...
static void free_role_datum_partially(struct policydb *db)
{
	...
	if (db->p_roles.table.htable) {
		hashtab_map(&db->p_roles.table,
			    destroy_role_datum_partially_callback, NULL);
				     ...
				     struct policydb *old_db)
{
	...
	new_type_attr_map_array =
		kvcalloc(sz, sizeof(struct ebitmap), GFP_KERNEL);
	...
	new_sym_val_to_name_types = kvcalloc(
		sz, sizeof(*new_db->sym_val_to_name[SYM_TYPES]), GFP_KERNEL);
	...
	ret = shallow_copy_hashtab(&new_db->p_types.table,
				   &old_db->p_types.table);
	if (ret < 0)
		goto out;
...
struct selinux_policy *ksu_dup_sepolicy(struct selinux_policy *old_pol)
{
	...
	struct policydb *new_db = &new_pol->policydb,
			*old_db = &old_pol->policydb;
